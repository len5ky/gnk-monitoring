...
def materialise_checks(
    ...
    for node in nodes:
        ...
        if template_override and template_override.exists():
            profile_data = load_yaml(template_override)
        else:
            ...
            profile_data = profiles_cache.get(profile_name, {})
        ...
        for entry in profile_data.get("ping", []):
            ...
            host = host.replace("${ADDRESS}", address)
...
def main() -> None:
    ...
    tasks = []
    for node_checks in materialise_checks(inventory_path, profiles_dir, template_override):
        tasks.append(
            loop.create_task(
                run_checks_for_node(
                    node_checks,
